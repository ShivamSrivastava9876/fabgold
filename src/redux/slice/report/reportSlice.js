    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getDailyReportDataAsync.fulfilled, (state, action) => {
                state.status = 'idle';
                if (action.payload) {
                    state.orderReportTableData.splice(0, 1, action.payload);
                    state.orderReportTableData = state.orderReportTableData[0].data;
                }
            })
...
export const getOrderReportList = (state) => state.report.orderReportTableData;
