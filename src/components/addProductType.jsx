    ...
    <>
      ...
      <div id="addProductType" className="flex justify-center m-8 relative">
        <form
          ...
          className="p-8 shadow-md flex flex-col justify-center items-center rounded-2xl md:w-34.125 bg-white relative"
        >
          <button
          ...
          <div className="grid md:grid-cols-1 gap-1">
            <div
              ...
              class={`relative inline-block cursor-pointer text-left mb-4 ${
                newCategory === "" && error ? "border-2 border-red-500" : ""
              }`}
            >
              <div class="inline-flex items-center justify-center text-xs md:text-sm font-semibold px-4 py-2 w-full h-3.3125 rounded-xl border border-gray-300 shadow-sm bg-white text-gray-700 hover:text-gray-500 focus:outline-none focus:ring focus:ring-indigo-200 active:bg-gray-100 active:text-gray-600">
                  ...
                  {categoryList.map((category) => (
                    <div key={category.id} class="py-1">
                      <div
                        href="#"
                        onClick={() =>
                          handleOptionClick(category.category_name)
                        }
            ...
            <div
              className={`mb-4 md:w-21.375 ${
                productType === "" && error ? "border-2 border-red-500" : ""
              }`}
            >
              <input
                type="text"
            ...
            <div
              className={`mb-4 flex justify-center items-center ${
                image === null && error ? "border-2 border-red-500" : ""
              }`}
            >
              <label
                htmlFor="fileInput"
                className="w-full flex items-center text-xs md:text-sm h-3.3125 py-2 px-8 border rounded-xl font-semibold outline-none border-[#9C9C9C] text-[#595858] cursor-pointer"
              >
                <FiImage className="mr-2" />{" "}
                {image !== null ? "Image uploaded" : "Upload image"}
                <input
                  type="file"
                  id="fileInput"
                  className="hidden"
